...
class Guild extends Base {
  constructor(data, client) {
    ...
    this.shard = client.shards.get(
      client.guildShardMap[this.id] || Base.getDiscordEpoch(data.id) % client.options.lastShardID || 0
    );
  ...
  get discoverySplashURL() {
    return this.discoverySplash
      ? this._client._formatImage(Endpoints.GUILD_DISCOVERY_SPLASH(this.id, this.discoverySplash))
      : null;
  }
  ...
  dynamicDiscoverySplashURL(format, size) {
    return this.discoverySplash
      ? this._client._formatImage(Endpoints.GUILD_DISCOVERY_SPLASH(this.id, this.discoverySplash), format, size)
      : null;
  }
