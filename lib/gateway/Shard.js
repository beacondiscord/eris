...
class Shard extends EventEmitter {
  ...
  checkReady() {
    if (!this.ready) {
      ...
      if (this.unsyncedGuilds > 0) return;
  ...
  wsEvent(packet) {
    switch (
        ...
        if (packet.d.user.username !== undefined) {
          ...
          if (
            ...
            (user.username !== packet.d.user.username ||
              ...
              user.banner !== packet.d.user.banner ||
              user.accentColor !== packet.d.user.accent_color)
          ) {
            oldUser = {
      ...
      case 'VOICE_STATE_UPDATE': {
        ...
        // This should mean that it's a DM call, so we're gonna avoid processing this.
        if (packet.d.guild_id === undefined) break;
        ...
        if (!member) {
          ...
          const channel = guild.channels.find(
            channel =>
              (channel.type === ChannelTypes.GUILD_VOICE || channel.type === ChannelTypes.GUILD_STAGE) &&
        ...
        if (oldChannelID != packet.d.channel_id) {
          ...
          if (oldChannelID) {
            ...
            if (
              ...
              oldChannel.type !== ChannelTypes.GUILD_STAGE
            ) {
          ...
          if (
            ...
            (newChannel.type === ChannelTypes.GUILD_VOICE || newChannel.type === ChannelTypes.GUILD_STAGE)
          ) {
      ...
      case 'TYPING_START': {
        ...
        if (this.client.listeners('typingStart').length > 0) {
          ...
          this.emit(
            'typingStart',
            this.client.getChannel(packet.d.channel_id) || { id: packet.d.channel_id },
      ...
      case 'GUILD_DELETE': {
        ...
        if (guild)
          ...
          for (var i = 0; i !== guild.channels.length; ++i) delete this.client.channelGuildMap[guild.channels[i].id];
        if (packet.d.unavailable)
          /**
      ...
      case 'CHANNEL_DELETE': {
        if (packet.d.type === ChannelTypes.DM || packet.d.type === undefined) {
          ...
          if (channel.type === ChannelTypes.GUILD_VOICE || channel.type === ChannelTypes.GUILD_STAGE) {
            for (var i = 0; i !== channel.voiceMembers.length; ++i) {
              const member = channel.voiceMembers[i];
          ...
          for (const voiceState of guild.pendingVoiceStates) {
            ...
            if (channel) {
              ...
              if (
                ...
                !this.client.voiceConnections.get(channel.guild ? channel.guild.id : 'call')
              ) {
      ...
      case 'STAGE_INSTANCE_DELETE': {
        ...
        this.emit(
          'stageInstanceDelete',
          guild.stageInstances.remove(packet.d) || new StageInstance(packet.d, this.client)
        );
        break;
      }
