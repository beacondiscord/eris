...
class CommandClient extends Client {
  ...
  async onMessageCreate(msg) {
    ...
    if (!msg.author) {
      this.emit(
        'warn',
        `Message ${msg.id} has author=${msg.author} | Channel ${msg.channel.id}, timestamp ${Date.now()}`
      );
      return;
    }
  ...
  registerCommandAlias(alias, label) {
    ...
    if (
      ...
      !(
        ...
        (caseInsensitiveLabel = this.commands[label.toLowerCase()].caseInsensitive)
      )
    ) {
  ...
  toJSON(props = []) {
    return super.toJSON(['commandOptions', 'guildPrefixes', 'commands', 'commandAliases', 'activeMessages', ...props]);
  }
}
