...
class Command {
  ...
  async process(args, msg) {
    const shouldDelete =
      this.deleteCommand && msg.channel.guild && msg.channel.permissionsOf(msg._client.user.id).has('manageMessages');
    ...
    if (this.cooldown !== 0 && !this.cooldownCheck(msg)) {
      ...
      if (
        ...
        (!this.cooldownReturns || this.cooldownAmounts[msg.author.id] <= this.cooldownReturns)
      ) {
    ...
    if (
      ...
      ((subcommand = this.subcommands[label.toLowerCase()]) !== undefined && subcommand.caseInsensitive)
    ) {
