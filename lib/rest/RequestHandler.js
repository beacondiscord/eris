...
class RequestHandler {
  constructor(client, options) {
    ...
    this._client = client;
    this.userAgent = `Beacon (https://beacon.wtf, ${require('../../package.json').version})`;
    this.ratelimits = {};
    this.latencyRef = {
  ...
  request(method, url, auth, body, file, _route, short) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const actualCall = cb => {
        ...
        try {
          if (auth) headers.Authorization = this._client._token;

          if (body?.reason) {
            const unencodedReason = body.reason;
          ...
          if (file) {
            if (Array.isArray(file)) {
              ...
              for (var i = 0; i != file.length; ++i) {
                const item = file[i];
                data.append(item.name, item.file, item.name);
              }

              if (body) data.append('payload_json', body);
            } else if (file.file) {
              data = new FormData();
              headers['Content-Type'] = 'multipart/form-data; boundary=' + data.boundary;
              ...
              if (body) {
                if (method === 'POST' && url.endsWith('/stickers')) {
                  for (const key in body) data.append(key, body[key]);
                } else data.append('payload_json', body);
              }
            ...
            if (method === 'GET' || method === 'DELETE') {
              ...
              for (var i = 0; i != k.length; ++i) {
                ...
                if (body[key] != undefined) {
                  if (Array.isArray(body[key]))
                    for (const val of body[key]) qs += `&${encodeURIComponent(key)}=${encodeURIComponent(val)}`;
              ...
              data = JSON.stringify(body, (_, v) => (typeof v === 'bigint' ? v.toString() : v));
        ...
        req.once('response', resp => {
          ...
          if (resp.headers['content-encoding']) {
            if (resp.headers['content-encoding'].includes('gzip')) _respStream = resp.pipe(Zlib.createGunzip());
          ...
          _respStream
            ...
            .once('end', () => {
              ...
              const retryAfter =
                parseInt(resp.headers['x-ratelimit-reset-after'] || resp.headers['retry-after']) * 1000;
              if (retryAfter >= 0) {
              ...
              if (resp.statusCode >= 300) {
                if (resp.statusCode === 429) {
                  ...
                  this._client.emit(
                    ...
                    } 429 (╯°□°）╯︵ ┻━┻: ${response}\n${content} ${now} ${route} ${resp.statusCode}: ${latency}ms (${
                ...
                let err;
                if (response.code) err = new DiscordRESTError(req, resp, response, stack);
                else err = new DiscordHTTPError(req, resp, response, stack);
        ...
        if (Array.isArray(data)) {
          for (var i = 0; i !== data.length; ++i) req.write(data[i]);
          req.end();
        } else req.end(data);
      };
