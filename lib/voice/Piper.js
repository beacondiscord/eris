      ...
      if (options.inlineVolume) {
        if (!options.format) {
          if (!this.converterCommand) {
            this.emit(
              'error',
              new Error('FFmpeg/avconv was not found on this system. Playback of this audio format is impossible')
            );
            this.reset();
            return false;
          }
          if (typeof source === 'string') {
            this.streams.push(
              (source = new FFmpegPCMTransformer({
        ...
        this.streams.push(
          (this.volume = source = source.pipe(new VolumeTransformer()).once('error', e => this.stop(e)))
        );
        this.volume.setVolume(this.volumeLevel);
        this.streams.push(
          this.volume
            .pipe(
              new PCMOpusTransformer({
        ...
        if (this.libopus) {
          if (typeof source === 'string') {
            this.streams.push(
              (source = new FFmpegOggTransformer({
                command: this.converterCommand,
                input: source,
                encoderArgs: options.encoderArgs,
                inputArgs: options.inputArgs,
