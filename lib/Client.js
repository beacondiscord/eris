...
class Client extends EventEmitter {
  ...
  constructor(token, options) {
    ...
    if (this.options.hasOwnProperty('intents')) {
      ...
      this.emit(
        'warn',
        '[DEPRECATED] Intents was not specified when initiating the client. You will only recieve Interaction events. If you want this, set intents to 0 or an empty array to remove this warning.'
      );
  ...
  addMessageReaction(channelID, messageID, reaction, userID) {
    ...
    return this.requestHandler.request(
      'PUT',
      Endpoints.CHANNEL_MESSAGE_REACTION_USER(channelID, messageID, reaction, userID || '@me'),
      true
    );
  }
  ...
  bulkEditCommandPermissions(guildID, permissions) {
    ...
    return this.requestHandler.request('PUT', Endpoints.GUILD_COMMAND_PERMISSIONS(this.application.id, guildID), true, {
      permissions
    });
  }
  ...
  async connect() {
    try {
      ...
      for (var i = this.options.firstShardID; i <= this.options.lastShardID; ++i) this.shards.spawn(i);
    } catch (err) {
  ...
  createGuildEvent(guildID, event) {
    return this.requestHandler
      .request('POST', Endpoints.GUILD_EVENT_CREATE(guildID), true, {
  ...
  createMessage(channelID, content, file) {
    if (content != undefined) {
      ...
      if (file) content = { payload_json: content };
    }
    return this.requestHandler
      .request('POST', Endpoints.CHANNEL_MESSAGES(channelID), true, content, file)
  ...
  createRole(guildID, options, reason) {
    if (options.permissions !== undefined) {
      options.permissions =
        options.permissions instanceof Permission ? String(options.permissions.allow) : String(options.permissions);
    }
    return this.requestHandler
  ...
  createThreadWithMessage(channelID, messageID, options) {
    return this.requestHandler
      .request('POST', Endpoints.THREAD_WITH_MESSAGE(channelID, messageID), true, {
        name: options.name,
  ...
  deleteWebhook(webhookID, token, reason) {
    return this.requestHandler.request(
      'DELETE',
      token ? Endpoints.WEBHOOK_TOKEN(webhookID, token) : Endpoints.WEBHOOK(webhookID),
      !token,
      {
        reason
      }
  ...
  disconnect(options) {
    this.ready = false;
    for (var i = 0; i !== this.shards.length; ++i) this.shards[i].disonnect(options);
  ...
  editAFK(afk) {
    ...
    for (var i = 0; i !== this.shards.length; ++i) this.shards[i].editAFK(afk);
  }
  ...
  editGuildEvent(event, eventID) {
    return this.requestHandler
      .request('POST', Endpoints.GUILD_EVENT(eventID), true, {
        channel_id: event.channelID,
        name: event.name,
        privacy_level: event.privacyLevel,
        scheduled_start_time: event.scheduledStartTime,
  ...
  editGuildTemplate(guildID, code, options) {
    return this.requestHandler
      .request('PATCH', Endpoints.GUILD_TEMPLATE_GUILD(guildID, code), true, options)
      .then(template => new GuildTemplate(template, this));
  }
  ...
  editMessage(channelID, messageID, content) {
    ...
    return this.requestHandler
      .request('PATCH', Endpoints.CHANNEL_MESSAGE(channelID, messageID), true, content, content.file)
      .then(message => new Message(message, this));
  }
  ...
  editStatus(status, activities) {
    ...
    for (var i = 0; i !== this.shards.length; ++i) this.shards[i].editStatus(status, activities);
  }
  ...
  editWebhookMessage(webhookID, token, messageID, options) {
    if (options.allowedMentions) options.allowed_mentions = this._formatAllowedMentions(options.allowedMentions);
  ...
  executeWebhook(webhookID, token, options) {
    ...
    return this.requestHandler
      .request(
        'POST',
        Endpoints.WEBHOOK_TOKEN(webhookID, token) + (options.wait ? '?wait=true' : ''),
        !!options.auth,
        options.file ? { payload_json: data } : data,
        options.file
      )
  ...
  getArchivedThreads(channelID, type, options = {}) {
    return this.requestHandler
      .request('GET', Endpoints.THREADS_ARCHIVED(channelID, type), true, options)
      .then(response => {
        return {
          hasMore: response.has_more,
          members: response.members.map(member => new ThreadMember(member, this)),
  ...
  getGuildBans(guildID) {
    return this.requestHandler.request('GET', Endpoints.GUILD_BANS(guildID), true).then(bans => {
      for (var i = 0; i !== bans.length; ++i) bans[i].user = new User(bans[i].user, this);
      return bans;
    });
  }
  ...
  getJoinedPrivateArchivedThreads(channelID, options = {}) {
    return this.requestHandler
      .request('GET', Endpoints.THREADS_ARCHIVED_JOINED(channelID), true, options)
      .then(response => {
        return {
          hasMore: response.has_more,
          members: response.members.map(member => new ThreadMember(member, this)),
  ...
  getMessageReaction(channelID, messageID, reaction, options = {}, before, after) {
    ...
    return this.requestHandler
      .request('GET', Endpoints.CHANNEL_MESSAGE_REACTION(channelID, messageID, reaction), true, options)
        ...
        if (limit <= messages.length) {
          return _after
            ? messages
                ...
                .concat(logs)
            : logs.concat(messages.slice(0, limit).map(message => new Message(message, this)));
        }
  ...
  getRESTGuildEvent(eventID) {
    if (!this.options.restMode) return Promise.reject(new Error('Eris REST mode is not enabled'));
    return this.requestHandler
      .request('GET', Endpoints.GUILD_EVENT(eventID), true)
      .then(data => new GuildEvent(data, this));
  }
  ...
  getVoiceRegions(guildID) {
    return guildID
      ? this.requestHandler.request('GET', Endpoints.GUILD_VOICE_REGIONS(guildID), true)
